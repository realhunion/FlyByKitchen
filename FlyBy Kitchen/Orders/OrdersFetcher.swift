...
class OrdersFetcher {
    ...
    func startMonitor() {
        ...
        self.listener = db.collection("Restaurants").document(myUID).collection("Orders").order(by: "timestamp", descending: true).limit(to: 20).addSnapshotListener { (snap, err) in
            ...
            for doc in docs {
                if let timestamp = doc.data()["timestamp"] as? Timestamp,
                    ...
                    let table = doc.data()["table"] as? Int,
